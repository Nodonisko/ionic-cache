...
describe('CacheService', () => {
  ...
  const blobKey = 'https://github.com/Nodonisko/ionic-cache-blob';
  const dataURL = 'data:image/png;base64,R0lGODlhDAAMAKIFAF5LAP/zxAAAANyuAP/gaP///wAAAAAAACH5BAEAAAUALAAAAAAMAAwAAAMlWLPcGjDKFYi9lxKBOaGcF35DhWHamZUW0K4mAbiwWtuf0uxFAgA7';
  ...
  it('should save blob item to storage (async)', async done => {
    try {
  ...
  it('should get previously stored blob value (async)', async done => {
    try {
      const response = await fetch(dataURL);
      const blob = await response.blob();
...
describe('Observable blob caching errors', () => {
  const key: string = 'http_cache_blob_error_test';

  let mockBlob: Blob = new Blob(['Hello, world!'], {type: 'text/plain'});

  let observableError = throwError(mockBlob);
...
describe('Observable Blob Caching', () => {
  const key: string = 'http_cache_blob_test';
